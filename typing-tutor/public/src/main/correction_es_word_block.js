class CorrectionEsWordBlock {
    ...
    if (this.activated === false) {
      return
		...
		const normalizedSpJoined = normalizeSpanish(txt_sp_joined);
		
		if (normalizedInput.startsWith(normalizedSpJoined)) {
			this.match_per = 1
	...
	sequentialMatch(arr, str) {
		...
		const normalizedStr = normalizeSpanish(str);
		
		for (let i = 0; i < arr.length; i++) {
			// Normalize each character for comparison
			const normalizedChar = normalizeSpanish(arr[i]);
			const found = normalizedStr.indexOf(normalizedChar, currentIndex);
