class Missle {
	...
	update(){
		if (this.firing) {
			this.y -= this.speed
			this.circle.y = this.y
			// Changed from size_kj to size_spanish for Spanish compatibility
			if (this.y <= (this.word.y + this.word.size_spanish)){
				this.hit()
			}
		}
	}
	...
	hit(){
		...
		this.firing = false
		this.reset_y_position()
		// Changed from romaji to spanish for Spanish compatibility
		this.scene.sound.play(this.word.spanish.replace(/ /g, '_'))
